...
<title>Feiertags-Wochenplaner V24</title>
<style>
...
/* V24 – kompakte Dispo-Oberfläche */
body{{background:#eef0f3;font-size:12px}}
.top{{background:rgba(255,255,255,.97);box-shadow:0 1px 0 rgba(20,22,26,.06)}}
.topin{{max-width:none;padding:9px 12px 8px}}
.headrow{{align-items:center}}
h1{{font-size:18px;font-weight:900}}
.sub{{font-size:10px;margin-top:2px}}
.filepill{{max-width:48vw;overflow:hidden;text-overflow:ellipsis;font-size:9px;padding:4px 7px;background:#f7f7f8}}
.controls.compactbar{{display:flex;align-items:center;gap:7px;margin-top:7px}}
.searchwrap{{flex:1;min-width:220px}}
.search{{height:34px;border-radius:8px;background:#f7f8fa}}
...
.filterdetails>summary::-webkit-details-marker,.moremenu>summary::-webkit-details-marker{{display:none}}
.filterdetails .chips{{position:absolute;top:40px;left:0;z-index:120;width:310px;padding:9px;background:#fff;border:1px solid var(--line);border-radius:10px;box-shadow:0 14px 35px rgba(0,0,0,.14)}}
.moremenu .menupop{{position:absolute;top:40px;right:0;z-index:120;width:180px;padding:7px;display:grid;gap:5px;background:#fff;border:1px solid var(--line);border-radius:10px;box-shadow:0 14px 35px rgba(0,0,0,.14)}}
.moremenu .btn{{width:100%;text-align:left}}
.primaryactions{{margin-left:auto;flex-wrap:nowrap}}
.viewfilter{{flex-wrap:nowrap}}
.viewbtn{{height:34px;padding:6px 9px}}
.btn{{min-height:34px;padding:6px 10px;border-radius:8px}}
main{{max-width:none;padding:9px 12px 22px}}
...
.outagedays{{display:flex;gap:4px;flex-wrap:wrap;align-items:center}}
.outageday{{height:34px;min-width:43px;padding:0 9px;border:1px solid var(--line);border-radius:8px;background:#f8f9fa;color:var(--text);font-size:10px;font-weight:900;cursor:pointer}}
.outageday:hover{{background:#fff}}
.outageday.active{{border-color:#6e5a9b;box-shadow:inset 0 0 0 1px #6e5a9b;background:var(--accent-soft)}}
.outageday.active.day-Mo{{background:#fff1bf}}.outageday.active.day-Die{{background:#dff4ff}}.outageday.active.day-Mitt{{background:#e7e0ff}}.outageday.active.day-Don{{background:#dff6e8}}.outageday.active.day-Fr{{background:#ffe1df}}.outageday.active.day-Sam{{background:#f2e2ff}}
.outagebadge{{display:inline-flex;align-items:center;margin-left:5px;padding:2px 5px;border-radius:999px;background:#9c3340;color:#fff;font-size:8px;font-weight:900;letter-spacing:.04em}}
.daycol.outage-selected{{outline:2px solid rgba(156,51,64,.28);outline-offset:-2px}}
.flowarrow{{height:34px;display:grid;place-items:center;color:#9a9ca3;font-weight:900}}
.bigaction{{height:34px}}
.flowhint{{margin-left:auto;align-self:center;font-size:9px;color:var(--muted);white-space:nowrap}}
.flowstatus{{display:inline-flex;align-items:center;gap:8px;border:1px solid var(--line);background:#fff;border-radius:8px;padding:7px 9px;font-size:10px;font-weight:800;white-space:nowrap}}
.flowstatus b{{font-size:12px;color:var(--text)}}
.flowstatus .done{{color:var(--good)}}
...
.settingspanel>summary::-webkit-details-marker,.collapsedchanges>summary::-webkit-details-marker{{display:none}}
...
.planningrules{{margin:0;border:0;border-radius:0;padding:9px 10px;background:#fafafa;grid-template-columns:180px minmax(340px,1fr) auto auto}}
.compactnotice{{margin:0;border-radius:0;border:0;border-top:1px solid var(--line2);border-left:0;background:#fafafa;padding:7px 10px;font-size:9px}}
.compactmetrics{{display:flex;gap:5px;margin-bottom:7px;overflow-x:auto;padding-bottom:1px}}
.compactmetrics .metric{{min-width:88px;flex:0 0 auto;border:0;background:transparent;padding:3px 7px;border-right:1px solid #d9dbe0;border-radius:0}}
.compactmetrics .metric:last-child{{border-right:0}}
.compactmetrics .v{{font-size:14px}}
.compactmetrics .k{{font-size:8px}}
.compactmetrics .issue .v{{color:var(--bad)}}
.slimunplanned{{margin-bottom:7px;border-style:solid}}
.slimunplanned .unplanned-head{{padding:5px 8px;background:#fff}}
.slimunplanned .unplanned-drop{{min-height:36px;max-height:88px;overflow:auto;padding:3px}}
.dropnote{{font-size:8px;color:var(--muted);text-transform:uppercase;letter-spacing:.08em;font-weight:900}}
.weekwrap{{overflow-x:auto;padding-bottom:0}}
.week{{grid-template-columns:repeat(6,minmax(195px,1fr));min-width:1180px;gap:5px}}
.daycol{{height:calc(100vh - 235px);min-height:520px}}
.dayhead{{padding:7px 8px;border-radius:8px 8px 0 0}}
.dayname{{font-size:13px}}
.daymeta{{font-size:8px}}
.addtour{{padding:4px 6px;font-size:9px}}
.daybody{{padding:4px;background:#e5e7ea;border-color:#d5d7db;border-radius:0 0 8px 8px}}
.route{{border-radius:7px;margin-bottom:4px;box-shadow:none}}
.route.changed{{box-shadow:inset 3px 0 0 var(--accent)}}
.route.newroute{{box-shadow:inset 3px 0 0 var(--accent);border-color:#c8bddc}}
.routehead{{padding:6px 6px;align-items:flex-start}}
.rtitle{{font-size:11px;gap:4px}}
.rmeta{{font-size:8px;line-height:1.25;white-space:normal;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}}
.route-right{{gap:2px;flex-wrap:wrap;justify-content:flex-end;max-width:116px}}
.seqbtn{{font-size:8px;padding:3px 4px;background:#f7f7f8}}
.source{{font-size:7px;padding:1px 4px}}
.delta{{font-size:8px;padding:2px 4px}}
.dropzone{{padding:2px;min-height:25px}}
.cust{{grid-template-columns:16px minmax(0,1fr) 14px;gap:4px;padding:5px;margin:3px 0;border-radius:6px}}
.cname{{font-size:9.5px}}
.csub{{font-size:8.5px}}
.geoline{{margin-top:3px;gap:2px}}
.fit,.flag{{font-size:7px;padding:1px 4px}}
.remove{{font-size:12px}}
.suggestions{{position:fixed;display:none;right:12px;top:88px;bottom:12px;width:min(620px,calc(100vw - 24px));z-index:140;margin:0;border-radius:12px;box-shadow:0 20px 60px rgba(0,0,0,.24)}}
.suggestions.show{{display:flex;flex-direction:column}}
.sugghead{{flex:0 0 auto;padding:10px 12px;background:#fff}}
.quickapply{{display:flex;gap:5px;flex-wrap:wrap;padding:7px 10px;border-bottom:1px solid var(--line2);background:#fafafa}}
.suggbody{{max-height:none;flex:1;overflow:auto}}
.iconclose{{border:0;background:#f0f1f3;border-radius:7px;width:30px;height:30px;font-size:18px;color:#666}}
.collapsedchanges{{margin-top:7px}}
.collapsedchanges .changetable{{font-size:9px}}
.visuals{{display:none}}
.drawer{{right:12px;bottom:12px;max-width:390px}}
.modal{{border-radius:14px}}
.visualmodal{{width:min(900px,calc(100vw - 30px))}}
.mapsmodal{{width:min(650px,calc(100vw - 30px))}}
@media(max-width:1250px){{.flowhint{{display:none}}.filepill{{display:none}}}}
@media(max-width:900px){{.primaryactions #finishBtn{{display:none}}.controls.compactbar{{flex-wrap:wrap}}.searchwrap{{flex-basis:100%}}.plannerflow{{align-items:stretch}}.flowfield{{flex:1}}.flowfield select{{width:100%;min-width:0}}.outagedays{{max-width:100%}}.daycol{{height:calc(100vh - 285px)}}}}
</style>
</head>
<body>
<div class="top"><div class="topin">
  <div class="headrow">
    <div><h1>Feiertagsplanung</h1><div class="sub">Woche verschieben · prüfen · exportieren</div></div>
    <div class="filepill">{src}{(" · " + aux) if aux else ""} · {generated}</div>
  </div>
  <div class="controls compactbar">
    ...
    <div class="viewfilter" id="viewFilter"><button class="viewbtn active" data-view="all" type="button">Alle</button><button class="viewbtn" data-view="new" type="button">Neu</button><button class="viewbtn" data-view="changed" type="button">Geändert</button></div>
    <div class="actions primaryactions"><button class="btn" id="finishBtn">Report</button><button class="btn primary" id="xlsxBtn">Excel exportieren</button></div>
    <details class="moremenu"><summary>Mehr</summary><div class="menupop"><button class="btn" id="resetBtn">Zurücksetzen</button><button class="btn" id="csvBtn">Änderungen CSV</button><button class="btn" onclick="window.print()">Drucken</button></div></details>
  </div>
</div></div>
<main>
  <section class="plannerbar">
    <div class="plannerflow">
      ...
      <div class="flowfield"><span>Zieltag für diesen Vorschlagslauf</span><select id="toDay"></select></div>
      <button class="btn primary bigaction" id="suggestBtn" type="button">Vorschläge</button>
      <div class="flowstatus" id="outageStatus"><span>offen <b id="openOutageCount">0</b></span><span class="done">bereits am Zieltag <b id="coveredOutageCount">0</b></span></div>
      <div class="flowhint">Mehrere Ausfalltage möglich · Zieltag gilt für den aktuellen Vorschlagslauf · Startbereich bleibt gleich</div>
    </div>
  ...
  <details class="settingspanel">
    ...
    <div class="planningrules">
      <div class="rulefield"><label for="maxCustomers">Max. Kunden je Tour</label><input id="maxCustomers" type="number" min="1" max="30" step="1" value="12"><div class="rulemeta">Harte Obergrenze. Vorschläge orientieren sich zusätzlich an vergleichbaren Touren.</div></div>
    ...
    <div class="notice compactnotice"><b>Zeit:</b> CSV-Start = Ladebeginn. +60 Min. Laden → Abfahrt. Valluhn = Stopp 0. Geo, Zeitrestriktionen, Tourgröße und Wochen-Referenzen werden berücksichtigt. Original-Doppelbelieferungen bleiben erlaubt; neue werden blockiert.</div>
  </details>

  <section class="suggestions" id="suggestions"><div class="sugghead"><div><strong id="suggTitle">Vorschläge</strong><div class="sub" id="suggSub"></div></div><button class="iconclose" id="closeSuggBtn" type="button" aria-label="Vorschläge schließen">×</button></div><div class="quickapply"><button class="btn" id="applyVeryGoodBtn" type="button">Sehr gute übernehmen</button><button class="btn primary" id="applyGoodBtn" type="button">Sehr gut + gut</button><button class="btn" id="applyRestAllBtn" type="button">Resttouren anlegen</button></div><div class="suggbody" id="suggBody"></div></section>
  ...
  <section class="unplanned slimunplanned">
    <div class="unplanned-head"><div><span class="unplanned-title">Ausgeplant</span><span class="sub"> · nur offene manuell ausgeplante Kunden; Zieltag-Doppelungen werden ausgeblendet</span></div><span class="dropnote">Ablage</span></div>
    <div id="unplannedDrop" class="unplanned-drop" data-route="UNPLANNED"></div>
  </section>

  <div class="weekwrap"><div id="week" class="week"></div></div>

  <details class="changes collapsedchanges">
...
let autoCoveredAids=new Set();
let outageDays=new Set(['Sam']);
...
function templateRankFor(r,a){{
  if(!r||!a)return null;
  if(a.originalRouteId===r.id&&Number.isFinite(Number(a.originalLoadOrder)))return Number(a.originalLoadOrder);
  if(r.referenceDay&&r.referenceTour){{
    const ref=routes.find(x=>x.day===r.referenceDay&&x.source===r.source&&String(x.tour)===String(r.referenceTour));
    const hit=ref?.items.find(x=>sapKey(x)===sapKey(a));
    if(hit){{const n=Number(hit.originalLoadOrder??hit.currentLoadOrder);if(Number.isFinite(n))return n;}}
  }}
  return null;
}}
function scheduleItems(items,r){{
  let time=departureMinutes(r),pos=DEPOT,remaining=[...items],ordered=[],violations=0,unknown=0;
  while(remaining.length){{
    const ranked=remaining.map(a=>templateRankFor(r,a)).filter(Number.isFinite),minTemplate=ranked.length?Math.min(...ranked):null;
    let best=null;
    for(const a of remaining){{
      const tr=travelMinutes(pos,a),raw=time+tr,w=timeWindowFor(a,r.day),wait=w.earliest!=null&&raw<w.earliest?w.earliest-raw:0,arrival=raw+wait,late=w.latest!=null&&arrival>w.latest?arrival-w.latest:0,slack=w.latest!=null?Math.max(0,w.latest-arrival):9999,geoBad=!Number.isFinite(a.lat)||!Number.isFinite(a.lon),rank=templateRankFor(r,a);
      let templatePenalty=12;
      if(Number.isFinite(rank)&&Number.isFinite(minTemplate))templatePenalty=Math.max(0,rank-minTemplate)*18;
      const score=(geoBad?5000:tr)+wait*.22+late*1000+(w.latest!=null?slack*.03:35)+(w.active?0:15)+templatePenalty;
      if(!best||score<best.score)best={{a,tr,w,arrival,late,score,geoBad,rank}};
    }}
    const idx=remaining.findIndex(x=>x.aid===best.a.aid);remaining.splice(idx,1);if(best.geoBad)unknown++;if(best.late>0)violations++;best.a._eta=best.arrival;best.a._timeWindow=best.w;best.a._timeViolation=best.late>0;ordered.push(best.a);time=best.arrival+SERVICE_MIN;pos=best.a;
  }}
  return {{ordered,violations,unknown,start:departureMinutes(r),finish:time}};
}}
...
function syncTargetDayOptions(){{
  const sel=document.getElementById('toDay');if(!sel)return;
  [...sel.options].forEach(o=>o.disabled=outageDays.has(o.value));
  if(!sel.value||outageDays.has(sel.value)){{
    const chosen=selectedOutageDays(),firstIdx=chosen.length?Math.min(...chosen.map(d=>DAY_ORDER.indexOf(d))):DAY_ORDER.length;
    let fallback=null;
    for(let i=firstIdx-1;i>=0;i--)if(!outageDays.has(DAY_ORDER[i])){{fallback=DAY_ORDER[i];break}}
    if(!fallback)fallback=DAY_ORDER.find(d=>!outageDays.has(d))||'';
    sel.value=fallback;
  }}
}}
function renderOutageButtons(){{
  const box=document.getElementById('outageDays');if(!box)return;
  box.innerHTML=DAY_ORDER.map(d=>`<button type="button" class="outageday day-${{d}} ${{outageDays.has(d)?'active':''}}" data-outage-day="${{d}}" aria-pressed="${{outageDays.has(d)?'true':'false'}}">${{DAY_LABELS[d].slice(0,2)}}</button>`).join('');
  box.querySelectorAll('[data-outage-day]').forEach(b=>b.addEventListener('click',()=>{{
    const d=b.dataset.outageDay;
    if(outageDays.has(d)){{if(outageDays.size===1){{showInfo('Mindestens ein Ausfalltag muss ausgewählt bleiben.');return}}outageDays.delete(d)}}else outageDays.add(d);
    renderOutageButtons();syncTargetDayOptions();autoResolveCoveredDeliveries(true);
...
function initControls(){{
  ...
  document.getElementById('toDay').value='Fr';renderOutageButtons();syncTargetDayOptions();
  ...
  document.getElementById('toDay').addEventListener('change',()=>autoResolveCoveredDeliveries(true));
  ...
  document.getElementById('applyRulesBtn').addEventListener('click',()=>{{clampMax();parseExcluded();autoResolveCoveredDeliveries(true);if(document.getElementById('suggestions').classList.contains('show'))computeSuggestions()}});
...
function buildOriginalPlan(){{
  assignments=[]; routes=[]; unplanned=[]; suggestions=[];restSuggestions=[];cancelledAids=new Set();cancelReasons=new Map();autoCoveredAids=new Set();document.getElementById('suggestions')?.classList.remove('show');
  CUSTOMERS.forEach(c=>{{
...
function restoreAutoCovered(){{
  if(!autoCoveredAids.size)return;
  [...autoCoveredAids].forEach(aid=>{{
    const i=unplanned.findIndex(a=>a.aid===aid);if(i<0)return;
    const a=unplanned.splice(i,1)[0];
    const r=getRoute(a.originalRouteId);
    if(r){{r.items.push(a);r.items.sort((x,y)=>{{const xo=Number(x.originalLoadOrder),yo=Number(y.originalLoadOrder),xv=Number.isFinite(xo),yv=Number.isFinite(yo);if(xv&&yv)return xo-yo;if(xv)return -1;if(yv)return 1;return (x.source_row??0)-(y.source_row??0)}});r.items.forEach((x,j)=>x.currentLoadOrder=j+1);}}
    cancelledAids.delete(aid);cancelReasons.delete(aid);
  }});
...
function updateOutageStatus(){{
  ...
  const open=fromDays.length&&toDay&&!daySet.has(toDay)?routes.filter(r=>daySet.has(r.day)).reduce((n,r)=>n+r.items.filter(a=>!isExcluded(a)).length,0)+unplanned.filter(a=>daySet.has(a.originalDay)&&!cancelledAids.has(a.aid)&&!isExcluded(a)).length:0;
  const excluded=routes.filter(r=>daySet.has(r.day)).reduce((n,r)=>n+r.items.filter(a=>isExcluded(a)).length,0);
  const o=document.getElementById('openOutageCount'),c=document.getElementById('coveredOutageCount');if(o)o.textContent=open;if(c)c.textContent=autoCoveredAids.size;
  const s=document.getElementById('outageStatus');if(s)s.title=(excluded?`${{excluded}} SAP auf Ausschlussliste bleiben unverändert. `:'')+(fromDays.length?`Ausfall: ${{outageDaysLabel(fromDays)}}.`:'');
}}
function autoResolveCoveredDeliveries(showToast=true){{
  restoreAutoCovered();
  const fromDays=selectedOutageDays(),toDay=document.getElementById('toDay')?.value,daySet=new Set(fromDays);
  if(!fromDays.length||!toDay||daySet.has(toDay)){{renderAll();return}}
  const eligible=routes.filter(r=>daySet.has(r.day)).flatMap(r=>r.items.map(a=>({{a,from:r}}))).filter(x=>!isExcluded(x.a)&&targetDayAlreadyHas(x.a,toDay));
  const touched=new Set();
  eligible.forEach(x=>{{const found=removeFromCurrent(x.a.aid);if(!found)return;if(found.from)touched.add(found.from.id);cancelledAids.add(found.a.aid);autoCoveredAids.add(found.a.aid);cancelReasons.set(found.a.aid,'Zieltag bereits vorhanden');unplanned.push(found.a)}});
  touched.forEach(id=>{{const r=getRoute(id);if(r)r.items.forEach((a,i)=>a.currentLoadOrder=i+1)}});
  renderAll();
  if(showToast&&eligible.length){{const d=document.getElementById('drawer');d.className='drawer show good';document.getElementById('drawerTitle').textContent='Bereits am Zieltag erledigt';document.getElementById('drawerText').textContent=`${{eligible.length}} Lieferung${{eligible.length===1?'':'en'}} automatisch aus ${{outageDaysLabel(fromDays)}} entfernt, weil dieselben SAP am ${{DAY_LABELS[toDay]}} bereits beliefert werden.`;clearTimeout(window.__drawerTimer);window.__drawerTimer=setTimeout(()=>d.className='drawer',4500)}}
}}
...
function routeHtml(r){{
  ...
  const refNote=r.referenceTour?` · Vorlage ${{DAY_LABELS[r.referenceDay]}} ${{esc(r.referenceTour)}}`:'';
  const meta=`${{r.items.length}}/${{maxCustomers}} Kunden`+(clock?` · ${{clock}}`:(start?` · Start ${{esc(start)}}`:''))+refNote+(tc?` · ⚠ ${{tc}} Zeitkonflikt${{tc===1?'':'e'}}`:'' );
  return `<section class="route ${{routeChanged(r)?'changed':''}} ${{r.manuallyCreated?'newroute':''}} ${{cap?'overcap':''}} ${{tc?'timeconflict':''}}" data-route="${{r.id}}"><div class="routehead"><div class="rleft"><div class="rtitle">Tour ${{esc(r.tour)}} ${{r.manuallyCreated?`<span class="newbadge">NEU</span>`:''}} <span class="source">${{esc(r.source.replace('HUPA_','HUPA '))}}</span> </div><div class="rmeta ${{cap?'capwarn':''}}">${{meta}}</div></div><div class="route-right">${{d!==0?`<span class="delta">${{d>0?'+':''}}${{d}}</span>`:''}}<button class="seqbtn" title="Aktuelle Tour in Google Maps öffnen" data-google-maps="${{r.id}}">Maps</button><button class="seqbtn" title="Tour grafisch anzeigen" data-show-visual="${{r.id}}">Karte</button><button class="seqbtn" title="Lade-/Fahrfolge ab Valluhn neu berechnen" data-seq-route="${{r.id}}">Sort.</button>${{r.manuallyCreated?`<button class="route-delete" title="Neue Tour löschen" data-delete-route="${{r.id}}">×</button>`:''}}</div></div><div class="dropzone" data-route="${{r.id}}">${{cards||'<div class="empty">Kunden hierher ziehen</div>'}}</div></section>`;
}}
...
function renderWeek(){{
  ...
  week.innerHTML=DAY_ORDER.map(day=>{{
    ...
    return `<div class="daycol ${{outageDays.has(day)?'outage-selected':''}}"><div class="dayhead day-${{day}}"><div class="dayhead-main"><div class="dayname">${{DAY_LABELS[day]}}${{outageDays.has(day)?'<span class="outagebadge">AUSFALL</span>':''}}</div><div class="daymeta">${{active.length}} Touren · ${{total}} Kunden</div></div><button class="addtour" type="button" data-add-tour="${{day}}">+ Tour</button></div><div class="daybody" data-day="${{day}}">${{rs.map(routeHtml).join('')||'<div class="empty">Keine Touren</div>'}}</div></div>`;
  }}).join('');
}}
...
function renderUnplanned(){{
  ...
  box.innerHTML=unplanned.filter(a=>!autoCoveredAids.has(a.aid)&&activeSources().includes(a.Quelle)&&(!searchText||[a.SAP,a.CSB,a.Name,a.Ort,a.Plz,a.Strasse].some(v=>String(v??'').toLowerCase().includes(searchText)))).map(a=>`<div class="cust origin-${{a.originalDay}} moved ${{isExcluded(a)?'excluded':''}}" draggable="true" data-aid="${{a.aid}}" title="Herkunft: ${{DAY_LABELS[a.originalDay]}}${{cancelledAids.has(a.aid)?' · '+(cancelReasons.get(a.aid)||'Ausfall-Lieferung gestrichen'):''}}"><div class="seq">–</div><div class="cmain"><div class="cname">${{a.SAP}} · ${{esc(a.Strasse||'')}}</div><div class="csub"><strong>${{esc(a.Name)}}</strong> · ${{esc(a.Ort)}}${{cancelledAids.has(a.aid)?' · <b>GESTRICHEN</b>':''}}</div></div></div>`).join('')||'<div class="empty">Keine offenen ausgeplanten Kunden</div>';
}}
...
function sapKey(a){{return String(a?.SAP??'').trim()}}
function routeSapSet(r){{return new Set((r?.items||[]).map(sapKey).filter(Boolean))}}
function routeCentroid(r){{
  const pts=(r?.items||[]).filter(a=>Number.isFinite(a.lat)&&Number.isFinite(a.lon));
  if(!pts.length)return null;
  return {{lat:pts.reduce((z,a)=>z+a.lat,0)/pts.length,lon:pts.reduce((z,a)=>z+a.lon,0)/pts.length}};
}}
function routePatternFit(fromRoute,candidate){{
  if(!fromRoute||!candidate||fromRoute.source!==candidate.source)return null;
  const a=routeSapSet(fromRoute),b=routeSapSet(candidate);if(!a.size||!b.size)return null;
  let overlap=0;a.forEach(s=>{{if(b.has(s))overlap++}});
  const ratio=overlap/Math.max(1,a.size),ca=routeCentroid(fromRoute),cb=routeCentroid(candidate),centroidKm=ca&&cb?dist(ca,cb):NaN;
  return {{overlap,ratio,centroidKm}};
}}
function bestPatternReference(fromRoute,fromDay,toDay){{
  if(!fromRoute)return null;
  const refs=routes.filter(r=>!r.manuallyCreated&&r.source===fromRoute.source&&r.items.length>0&&r.day!==fromDay&&r.day!==toDay);
  const cand=refs.map(r=>{{const p=routePatternFit(fromRoute,r);if(!p)return null;const geo=Number.isFinite(p.centroidKm)?p.centroidKm:999;const quality=p.overlap*10000+p.ratio*5000-Math.min(geo,250);return {{r,p,quality}}}}).filter(Boolean).sort((x,y)=>y.quality-x.quality);
  const strong=cand.find(x=>x.p.overlap>=2||x.p.ratio>=.25);
  return strong||null;
}}
function directWeekReferences(a,fromDay,toDay){{
  const sap=sapKey(a);if(!sap)return [];
  return routes.filter(r=>!r.manuallyCreated&&r.source===a.Quelle&&r.day!==fromDay&&r.day!==toDay&&r.items.some(x=>sapKey(x)===sap));
}}
function bestWeekReference(a,fromDay,toDay,fromRoute=null,openEntries=[]){{
  const direct=directWeekReferences(a,fromDay,toDay);
  if(direct.length){{
    const ranked=direct.map(r=>{{
      const f=referenceGeoFit(a,r),p=routePatternFit(fromRoute,r)||{{overlap:0,ratio:0,centroidKm:NaN}},openOverlap=openPoolOverlap(r,openEntries);
      const score=-(openOverlap*7500+p.overlap*3500+p.ratio*1800)+(Number.isFinite(f.centroid)?f.centroid:999);
      return {{r,f,p:{{...p,openOverlap}},score,kind:'same_customer'}};
    }}).sort((x,y)=>x.score-y.score);
    return ranked[0];
  }}
  const pattern=bestPatternReference(fromRoute,fromDay,toDay);
  if(pattern){{const f=referenceGeoFit(a,pattern.r);if(f.key!=='bad'&&f.key!=='unknown')return {{r:pattern.r,f,p:{{...pattern.p,openOverlap:openPoolOverlap(pattern.r,openEntries)}},score:fitRank(f.key)*10000+(Number.isFinite(f.nearest)?f.nearest:999),kind:'tour_pattern'}}}}
  ...
  const cand=refs.map(r=>{{const f=referenceGeoFit(a,r),openOverlap=openPoolOverlap(r,openEntries);return {{r,f,p:{{...(routePatternFit(fromRoute,r)||{{}}),openOverlap}},score:fitRank(f.key)*10000+(Number.isFinite(f.nearest)?f.nearest:999)*.7+(Number.isFinite(f.centroid)?f.centroid:999)*.3-openOverlap*2500,kind:'geo'}}}}).sort((x,y)=>x.score-y.score);
  const known=cand.filter(x=>x.f.key!=='unknown');
  return (known.length?known:cand)[0]||null;
}}
function targetReferenceAffinity(target,weekRef){{
  if(!target||!weekRef?.r||target.source!==weekRef.r.source)return {{bonus:0,shared:0,centroidKm:NaN}};
  const a=routeSapSet(target),b=routeSapSet(weekRef.r);let shared=0;a.forEach(s=>{{if(b.has(s))shared++}});
  const ca=routeCentroid(target),cb=routeCentroid(weekRef.r),centroidKm=ca&&cb?dist(ca,cb):NaN;
  let bonus=Math.min(7500,shared*1800);
  if(Number.isFinite(centroidKm)){{if(centroidKm<=20)bonus+=3500;else if(centroidKm<=40)bonus+=2200;else if(centroidKm<=65)bonus+=900}}
  return {{bonus:Math.min(11000,bonus),shared,centroidKm}};
}}
function sharedHistoricTourDays(a,b,fromDay,toDay){{
  let n=0;const days=[];
  DAY_ORDER.forEach(d=>{{if(d===fromDay||d===toDay)return;const ta=a?.[d],tb=b?.[d];if(ta!=null&&ta!==''&&tb!=null&&tb!==''&&String(ta)===String(tb)){{n++;days.push(d)}}}});
  return {{count:n,days}};
}}
function targetDnaAffinity(a,target,fromDay,toDay){{
  if(!a||!target||a.Quelle!==target.source)return {{bonus:0,peers:0,sharedDays:0,maxDays:0}};
  let peers=0,sharedDays=0,maxDays=0;
  target.items.forEach(b=>{{if(sapKey(a)===sapKey(b))return;const h=sharedHistoricTourDays(a,b,fromDay,toDay);if(h.count>0){{peers++;sharedDays+=h.count;maxDays=Math.max(maxDays,h.count)}}}});
  let bonus=sharedDays*2200+peers*900+maxDays*700;
  bonus=Math.min(13500,bonus);
  return {{bonus,peers,sharedDays,maxDays}};
}}
function openPoolOverlap(refRoute,openEntries){{
  if(!refRoute||!openEntries?.length)return 0;const set=routeSapSet(refRoute);let n=0;openEntries.forEach(x=>{{if(set.has(sapKey(x.a)))n++}});return n;
}}
function targetDayDigit(day){{return {{Mo:'1',Die:'2',Mitt:'3',Don:'4',Fr:'5',Sam:'6'}}[day]||''}}
function referenceBasedTourName(refTour,toDay,source){{
  const raw=String(refTour||'').trim(),digit=targetDayDigit(toDay);
  if(digit&&/^\d{{4,6}}$/.test(raw)&&/^[1-6]/.test(raw)){{
    const base=digit+raw.slice(1);if(!routes.some(r=>r.day===toDay&&r.source===source&&String(r.tour)===base))return base;
    let n=1;while(routes.some(r=>r.day===toDay&&r.source===source&&String(r.tour)===base+'N'+n))n++;return base+'N'+n;
  }}
  return 'REF-'+raw;
}}
function weekReferenceLabel(wr){{
  if(!wr)return 'Keine passende Wochen-Referenz gefunden.';
  const base=`${{DAY_LABELS[wr.r.day]}} · Tour ${{esc(wr.r.tour)}}`;
  if(wr.kind==='same_customer')return `<b>Direkte Wochen-Referenz:</b> ${{base}} · derselbe SAP fährt dort bereits${{wr.p?.openOverlap?` · ${{wr.p.openOverlap}} aktuell offene Kunden kennen diese Tour`:''}}${{wr.p?.overlap?` · ${{wr.p.overlap}} Kunde(n) der Ausfalltour ebenfalls dort`:''}}`;
  if(wr.kind==='tour_pattern')return `<b>Bestehendes Tourmuster:</b> ${{base}} · ${{wr.p?.overlap||0}} gemeinsame Kunden (${{Math.round((wr.p?.ratio||0)*100)}} % der Ausfalltour)${{wr.p?.openOverlap?` · ${{wr.p.openOverlap}} aktuell offene Kunden`:''}}`;
  return `<b>Geo-Referenz:</b> ${{base}} · ${{esc(wr.f.label)}}${{Number.isFinite(wr.f.nearest)?` · nächster Kunde ${{fmt(wr.f.nearest)}} km`:''}}`;
}}
...
function computeSuggestions(){{
  ...
  if(!fromDays.length){{box.classList.add('show');document.getElementById('suggTitle').textContent='Kein Ausfalltag gewählt';document.getElementById('suggSub').textContent='Bitte mindestens einen Wochentag anklicken.';document.getElementById('suggBody').innerHTML='';return}}
  if(daySet.has(toDay)){{box.classList.add('show');document.getElementById('suggTitle').textContent='Zieltag ist ebenfalls Ausfalltag';document.getElementById('suggSub').textContent='Bitte einen Zieltag wählen, der nicht als Ausfall markiert ist.';document.getElementById('suggBody').innerHTML='';return}}
  const allSrc=fromDays.flatMap(currentItemsForDay),src=allSrc.filter(x=>!isExcluded(x.a));
  ...
  suggestions=src.map(entry=>{{
    const a=entry.a,from=entry.from,originDay=from.day,weekRef=bestWeekReference(a,originDay,toDay,from,src);
    let cand=targets.filter(r=>r.source===a.Quelle).map(r=>{{const f=fitFor(a,r),affinity=targetReferenceAffinity(r,weekRef),dna=targetDnaAffinity(a,r,originDay,toDay);return {{r,f,affinity,dna,score:fitScore(f,r)-affinity.bonus-dna.bonus}}}}).sort((x,y)=>x.score-y.score);
    const known=cand.filter(x=>x.f.key!=='unknown');
  ...
  const groups={{}};
  leftover.forEach(s=>{{
    const wr=s.weekRef,usable=wr&&['same_customer','tour_pattern'].includes(wr.kind)||wr&&['good','ok','warn'].includes(wr.f.key),key=usable?s.a.Quelle+'::REF::'+wr.r.day+'::'+wr.r.tour:s.a.Quelle+'::ORIG::'+s.from.day+'::'+s.from.tour;
    if(!groups[key])groups[key]={{source:s.a.Quelle,originalTour:s.from.tour,originalDay:s.from.day,referenceDay:usable?wr.r.day:null,referenceTour:usable?wr.r.tour:null,referenceFit:usable?wr.f:null,referenceKind:usable?wr.kind:null,referencePattern:usable?wr.p:null,items:[]}};
  ...
  document.getElementById('suggTitle').textContent=`${{outageDaysLabel(fromDays)}} ${{fromDays.length===1?'fällt':'fallen'}} aus → ${{DAY_LABELS[toDay]}}`;
  const tcrit=src.filter(x=>x.a.Zeitkritisch).length,targetExistingCount=suggestions.filter(x=>x.targetExisting).length;
  const directRefs=suggestions.filter(x=>x.weekRef?.kind==='same_customer').length,patternRefs=suggestions.filter(x=>x.weekRef?.kind==='tour_pattern').length;
  ...
  document.getElementById('suggSub').textContent=`${{src.length}} offene Kunden aus ${{fromDays.length}} Ausfalltag${{fromDays.length===1?'':'en'}} · ${{directRefs}} mit direkter Wochen-Tour · ${{patternRefs}} über bestehendes Tourmuster · ${{targetExistingCount}} am Zieltag bereits vorhanden · ${{excludedCount}} SAP ausgeschlossen · ${{tcrit}} Zeit-Hinweise. Zieltag-Touren werden nach Tour-DNA, Wochen-Vorlage, Geo, Kapazität und Zeitrestriktionen bewertet. ${{refs}}`;
...
function buildRestSuggestion(g,toDay){{
  ...
  const stem=g.referenceTour?referenceBasedTourName(g.referenceTour,toDay,g.source):`REST-${{String(g.originalTour)}}`;
  ...
  return {{...g,toDay,name,compact,normalStart:starts.join(' / '),timeCount,referenceDay:g.referenceDay||null,referenceTour:g.referenceTour||null,referenceKind:g.referenceKind||null,referencePattern:g.referencePattern||null}};
}}
function suggestionRow(sg){{
  const a=sg.a,b=sg.best,wr=sg.weekRef;
  const weekRef=`<div class="suggalt">${{weekReferenceLabel(wr)}}</div>`;
  ...
  const aff=b.affinity&&b.affinity.bonus>0?`<div class="suggalt"><b>Wochenbezug Ziel-Tour:</b> ${{b.affinity.shared}} gemeinsame Kunden mit der Vorlage${{Number.isFinite(b.affinity.centroidKm)?` · Gebiete ca. ${{fmt(b.affinity.centroidKm)}} km auseinander`:''}}</div>`:'';
  const dna=b.dna&&b.dna.bonus>0?`<div class="suggalt"><b>Tour-DNA:</b> ${{b.dna.peers}} Kunden der Ziel-Tour fuhren mit diesem Kunden bereits zusammen · ${{b.dna.sharedDays}} gemeinsame Tourtage</div>`:'';
  ...
  const geo=`<span class="fit ${{f.key}}">${{esc(f.label)}}</span> ${{Number.isFinite(f.nearest)?`<span class="suggalt">nächster Kunde ${{fmt(f.nearest)}} km</span>`:''}} ${{f.dupe?'<span class="flag dupe">bereits Zieltag</span>':''}} ${{tinfo}} ${{timing}}<div class="suggalt">Tourgröße ${{b.r.items.length}} → ${{b.r.items.length+1}} · Wochen-Richtgröße ${{ref.typical}} · Auto-Limit ${{ref.soft}}</div>${{clock?`<div class="suggalt">${{esc(clock)}} · Start 0 = Valluhn</div>`:(st?`<div class="suggalt">Ladebeginn ${{esc(st)}} · Abfahrt +60 Min · Start 0 = Valluhn</div>`:'')}}${{aff}}${{dna}}${{weekRef}}`;
...
function restSuggestionRow(g,i){{
  ...
  const ref=g.referenceTour?`<div class="restmeta"><b>${{g.referenceKind==='same_customer'?'Direkte Wochen-Vorlage':g.referenceKind==='tour_pattern'?'Bestehendes Tourmuster':'Geo-Vorlage'}}:</b> ${{DAY_LABELS[g.referenceDay]}} · Tour ${{esc(g.referenceTour)}}${{g.referencePattern?.overlap?` · ${{g.referencePattern.overlap}} gemeinsame Kunden`:''}}</div>`:`<div class="restmeta">Keine passende Wochen-Vorlage; Gruppierung nach Ursprungstour ${{esc(g.originalTour)}}.</div>`;
...
function applyRestSuggestion(index){{
  ...
  const r={{id,day:g.toDay,source:g.source,tour:g.name,items:[],originalCount:0,manuallyCreated:true,normalStart:g.normalStart,generatedRest:true,referenceDay:g.referenceDay||null,referenceTour:g.referenceTour||null,referenceKind:g.referenceKind||null,referencePattern:g.referencePattern||null}};
  routes.push(r);
...
function bulkCancelOutage(){{
  const fromDays=selectedOutageDays(),toDay=document.getElementById('toDay').value,daySet=new Set(fromDays);
  if(!fromDays.length||daySet.has(toDay)){{alert('Bitte gültige Ausfalltage und einen anderen Zieltag wählen.');return}}
  const all=routes.filter(r=>daySet.has(r.day)).flatMap(r=>r.items.map(a=>({{a,from:r}})));
  ...
  const msg=`${{eligible.length}} Ausfall-Lieferung${{eligible.length===1?'':'en'}} aus ${{outageDaysLabel(fromDays)}} streichen, weil dieselben SAP am ${{DAY_LABELS[toDay]}} bereits eine Lieferung haben?`+(excludedMatches?`\n\n${{excludedMatches}} passende SAP aus der Ausschlussliste bleiben unverändert.`:'')+`\n\nNur diese Zieltag-Doppelungen werden gestrichen. Alle übrigen Kunden der Ausfalltage bleiben für Tourvorschläge erhalten.`;
  if(!confirm(msg))return;
  const touched=new Set();eligible.forEach(x=>{{const found=removeFromCurrent(x.a.aid);if(!found)return;if(found.from)touched.add(found.from.id);cancelledAids.add(found.a.aid);cancelReasons.set(found.a.aid,'Zieltag bereits vorhanden');unplanned.push(found.a)}});
  touched.forEach(id=>{{const r=getRoute(id);if(r)r.items.forEach((a,i)=>a.currentLoadOrder=i+1)}});
  renderAll();
  if(document.getElementById('suggestions').classList.contains('show'))computeSuggestions();
  showCancelledBulk(eligible.length,excludedMatches,fromDays,toDay);
}}
function showCancelledBulk(n,skipped,fromDays,toDay){{const d=document.getElementById('drawer');d.className='drawer show warn';document.getElementById('drawerTitle').textContent='Zieltag bereits vorhanden';document.getElementById('drawerText').textContent=`${{n}} Ausfall-Lieferung${{n===1?'':'en'}} aus ${{outageDaysLabel(fromDays)}} gestrichen, weil die SAP am ${{DAY_LABELS[toDay]}} bereits beliefert werden.`+(skipped?` ${{skipped}} ausgeschlossene SAP blieben unverändert.`:'');clearTimeout(window.__drawerTimer);window.__drawerTimer=setTimeout(()=>d.className='drawer',6000)}}
...
function renderMetrics(){{
  ...
  document.getElementById('mOut').textContent=unplanned.filter(a=>!autoCoveredAids.has(a.aid)).length;
  document.getElementById('mCancelled').textContent=autoCoveredAids.size;
  updateOutageStatus();
...
function currentPlanStats(){{
  const changed=changedRows(),dups=findDuplicateDeliveries(),newRoutes=routes.filter(r=>r.manuallyCreated),timeChanged=changed.filter(x=>x.a.Zeitkritisch),fromDays=selectedOutageDays(),daySet=new Set(fromDays);
  const leftOnOutage=routes.filter(r=>daySet.has(r.day)).reduce((n,r)=>n+r.items.length,0);
  return {{changed,dups,allowedDups:findAllowedOriginalDuplicates(),newRoutes,timeChanged,leftOnOutage,fromDays,unplannedCount:unplanned.length,cancelledCount:unplanned.filter(a=>cancelledAids.has(a.aid)).length}};
}}
...
def main():
    ...
    st.set_page_config(page_title="Feiertags-Wochenplaner V24 – HTML Generator", page_icon="📅", layout="wide")
    st.markdown(
        """
        <style>
    ...
    st.title("Feiertags-Wochenplaner V24 – HTML Generator")
    st.caption("Excel + Normal_Tourenstart.csv + Kisoft_Kunden.csv hochladen → kompakte Dispo-HTML erzeugen. V24 priorisiert direkte Wochen-Referenzen desselben SAP und bestehende Tourmuster aus der gesamten Woche und als Geo-Skizze ab Valluhn visualisiert. Zusätzlich besitzt jede Tour einen Visualisieren-Button und einen Google-Maps-Button für die aktuelle Reihenfolge. ein oder mehrere Ausfalltage, Zieltag, Startbereich, Zeitrestriktionen, Tourgröße, Ladezeit und Ladefolge bleiben berücksichtigt.")
    ...
    if upload is None or starts_upload is None or info_upload is None:
        st.info("Für V24 bitte alle drei aktuellen Dateien hochladen. Benötigte Excel-Blätter: DIREKT, MK, HUPA_NMS und HUPA_MALCHOW.")
        return
    ...
    st.success("HTML wurde erzeugt. V24 prüft zuerst, ob derselbe SAP an anderen Tagen bereits auf einer Tour läuft, danach bestehende Tourmuster mit gemeinsamen Kunden und erst zuletzt reine Geo-Ähnlichkeit. Neue Touren werden zusätzlich als Geo-Skizze ab Valluhn dargestellt.")
    st.download_button(
        "Feiertags_Wochenplaner.html herunterladen",
        data=html.encode("utf-8"),
        file_name="Feiertags_Wochenplaner_V24.html",
        mime="text/html",
