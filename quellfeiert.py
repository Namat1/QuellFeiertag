...
<title>Feiertags-Wochenplaner V22</title>
<style>
...
function bestWeekReference(a,fromDay,toDay,fromRoute=null){{
  const direct=directWeekReferences(a,fromDay,toDay);
  if(direct.length){{
    const ranked=direct.map(r=>{{const f=referenceGeoFit(a,r),p=routePatternFit(fromRoute,r)||{{overlap:0,ratio:0,centroidKm:NaN}};const score=-(p.overlap*5000+p.ratio*2500)+(Number.isFinite(f.centroid)?f.centroid:999);return {{r,f,p,score,kind:'same_customer'}}}}).sort((x,y)=>x.score-y.score);
    return ranked[0];
  }}
  const pattern=bestPatternReference(fromRoute,fromDay,toDay);
  if(pattern){{const f=referenceGeoFit(a,pattern.r);if(f.key!=='bad'&&f.key!=='unknown')return {{r:pattern.r,f,p:pattern.p,score:fitRank(f.key)*10000+(Number.isFinite(f.nearest)?f.nearest:999),kind:'tour_pattern'}}}}
  ...
  const cand=refs.map(r=>{{const f=referenceGeoFit(a,r);return {{r,f,p:routePatternFit(fromRoute,r),score:fitRank(f.key)*10000+(Number.isFinite(f.nearest)?f.nearest:999)*.7+(Number.isFinite(f.centroid)?f.centroid:999)*.3,kind:'geo'}}}}).sort((x,y)=>x.score-y.score);
...
function weekReferenceLabel(wr){{
  ...
  if(wr.kind==='same_customer')return `<b>Direkte Wochen-Referenz:</b> ${{base}} · derselbe SAP fährt dort bereits${{wr.p?.overlap?` · ${{wr.p.overlap}} Kunde(n) der Ausfalltour ebenfalls dort`:''}}`;
  if(wr.kind==='tour_pattern')return `<b>Bestehendes Tourmuster:</b> ${{base}} · ${{wr.p?.overlap||0}} gemeinsame Kunden (${{Math.round((wr.p?.ratio||0)*100)}} % der offenen Ausfalltour)`;
...
function computeSuggestions(){{
  ...
  suggestions=src.map(entry=>{{
    const a=entry.a,from=entry.from,weekRef=bestWeekReference(a,fromDay,toDay,from);
    let cand=targets.filter(r=>r.source===a.Quelle).map(r=>{{const f=fitFor(a,r),affinity=targetReferenceAffinity(r,weekRef);return {{r,f,affinity,score:fitScore(f,r)-affinity.bonus}}}}).sort((x,y)=>x.score-y.score);
    const known=cand.filter(x=>x.f.key!=='unknown');
...
function suggestionRow(sg){{
  ...
  const geo=`<span class="fit ${{f.key}}">${{esc(f.label)}}</span> ${{Number.isFinite(f.nearest)?`<span class="suggalt">nächster Kunde ${{fmt(f.nearest)}} km</span>`:''}} ${{f.dupe?'<span class="flag dupe">bereits Zieltag</span>':''}} ${{tinfo}} ${{timing}}<div class="suggalt">Tourgröße ${{b.r.items.length}} → ${{b.r.items.length+1}} · Wochen-Richtgröße ${{ref.typical}} · Auto-Limit ${{ref.soft}}</div>${{clock?`<div class="suggalt">${{esc(clock)}} · Start 0 = Valluhn</div>`:(st?`<div class="suggalt">Ladebeginn ${{esc(st)}} · Abfahrt +60 Min · Start 0 = Valluhn</div>`:'')}}${{aff}}${{weekRef}}`;
...
def main():
    ...
    st.set_page_config(page_title="Feiertags-Wochenplaner V22 – HTML Generator", page_icon="📅", layout="wide")
    st.markdown(
        """
        <style>
    ...
    if upload is None or starts_upload is None or info_upload is None:
        st.info("Für V22 bitte alle drei aktuellen Dateien hochladen. Benötigte Excel-Blätter: DIREKT, MK, HUPA_NMS und HUPA_MALCHOW.")
        return
    ...
    st.success("HTML wurde erzeugt. V22 prüft zuerst, ob derselbe SAP an anderen Tagen bereits auf einer Tour läuft, danach bestehende Tourmuster mit gemeinsamen Kunden und erst zuletzt reine Geo-Ähnlichkeit. Neue Touren werden zusätzlich als Geo-Skizze ab Valluhn dargestellt.")
    st.download_button(
        "Feiertags_Wochenplaner.html herunterladen",
        data=html.encode("utf-8"),
        file_name="Feiertags_Wochenplaner_V22.html",
        mime="text/html",
