...
<title>Feiertags-Wochenplaner V23</title>
<style>
...
@media(max-width:900px){{.primaryactions #finishBtn{{display:none}}.controls.compactbar{{flex-wrap:wrap}}.searchwrap{{flex-basis:100%}}.plannerflow{{align-items:stretch}}.flowfield{{flex:1}}.flowfield select{{width:100%;min-width:0}}.daycol{{height:calc(100vh - 285px)}}}}
</style>
</head>
<body>
...
<main>
  <section class="plannerbar">
    <div class="plannerflow">
      <div class="flowfield"><span>Ausfalltag</span><select id="fromDay"></select></div>
      ...
      <div class="flowhint">Zieltag-Doppelungen werden automatisch erledigt · Startbereich bleibt gleich</div>
    </div>
...
function initControls(){{
  ...
  document.getElementById('fromDay').addEventListener('change',()=>autoResolveCoveredDeliveries(true));
...
function updateOutageStatus(){{
  ...
  const open=fromDay&&toDay&&fromDay!==toDay?routes.filter(r=>r.day===fromDay).reduce((n,r)=>n+r.items.filter(a=>!isExcluded(a)).length,0)+unplanned.filter(a=>a.originalDay===fromDay&&!cancelledAids.has(a.aid)&&!isExcluded(a)).length:0;
  const excluded=fromDay?routes.filter(r=>r.day===fromDay).reduce((n,r)=>n+r.items.filter(a=>isExcluded(a)).length,0):0;
  ...
  const s=document.getElementById('outageStatus');if(s)s.title=excluded?`${{excluded}} SAP auf Ausschlussliste bleiben unverändert.`:'';
}}
function autoResolveCoveredDeliveries(showToast=true){{
  restoreAutoCovered();
  const fromDay=document.getElementById('fromDay')?.value,toDay=document.getElementById('toDay')?.value;
  if(!fromDay||!toDay||fromDay===toDay){{renderAll();return}}
  const eligible=routes.filter(r=>r.day===fromDay).flatMap(r=>r.items.map(a=>({{a,from:r}}))).filter(x=>!isExcluded(x.a)&&targetDayAlreadyHas(x.a,toDay));
  ...
  renderAll();
  if(showToast&&eligible.length){{const d=document.getElementById('drawer');d.className='drawer show good';document.getElementById('drawerTitle').textContent='Bereits am Zieltag erledigt';document.getElementById('drawerText').textContent=`${{eligible.length}} Lieferung${{eligible.length===1?'':'en'}} automatisch aus ${{DAY_LABELS[fromDay]}} entfernt, weil dieselben SAP am ${{DAY_LABELS[toDay]}} bereits beliefert werden.`;clearTimeout(window.__drawerTimer);window.__drawerTimer=setTimeout(()=>d.className='drawer',4500)}}
}}
...
function computeSuggestions(){{
  ...
  suggestions=src.map(entry=>{{
    const a=entry.a,from=entry.from,weekRef=bestWeekReference(a,fromDay,toDay,from,src);
    let cand=targets.filter(r=>r.source===a.Quelle).map(r=>{{const f=fitFor(a,r),affinity=targetReferenceAffinity(r,weekRef),dna=targetDnaAffinity(a,r,fromDay,toDay);return {{r,f,affinity,dna,score:fitScore(f,r)-affinity.bonus-dna.bonus}}}}).sort((x,y)=>x.score-y.score);
    const known=cand.filter(x=>x.f.key!=='unknown');
  ...
  const groups={{}};
  leftover.forEach(s=>{{
    const wr=s.weekRef,usable=wr&&['same_customer','tour_pattern'].includes(wr.kind)||wr&&['good','ok','warn'].includes(wr.f.key),key=usable?s.a.Quelle+'::REF::'+wr.r.day+'::'+wr.r.tour:s.a.Quelle+'::ORIG::'+s.from.tour;
    if(!groups[key])groups[key]={{source:s.a.Quelle,originalTour:s.from.tour,referenceDay:usable?wr.r.day:null,referenceTour:usable?wr.r.tour:null,referenceFit:usable?wr.f:null,referenceKind:usable?wr.kind:null,referencePattern:usable?wr.p:null,items:[]}};
  ...
  document.getElementById('suggSub').textContent=`${{src.length}} offene Kunden · ${{directRefs}} mit direkter Wochen-Tour · ${{patternRefs}} über bestehendes Tourmuster · ${{targetExistingCount}} am Zieltag bereits vorhanden · ${{excludedCount}} SAP ausgeschlossen · ${{tcrit}} Zeit-Hinweise. Zieltag-Touren werden nach Tour-DNA (gemeinsame Fahrten mit vorhandenen Kunden), Wochen-Vorlage, Geo, Kapazität und Zeitrestriktionen bewertet. ${{refs}}`;
...
def main():
    ...
    st.set_page_config(page_title="Feiertags-Wochenplaner V23 – HTML Generator", page_icon="📅", layout="wide")
    st.markdown(
        """
        <style>
    ...
    if upload is None or starts_upload is None or info_upload is None:
        st.info("Für V23 bitte alle drei aktuellen Dateien hochladen. Benötigte Excel-Blätter: DIREKT, MK, HUPA_NMS und HUPA_MALCHOW.")
        return
    ...
    st.success("HTML wurde erzeugt. V23 prüft zuerst, ob derselbe SAP an anderen Tagen bereits auf einer Tour läuft, danach bestehende Tourmuster mit gemeinsamen Kunden und erst zuletzt reine Geo-Ähnlichkeit. Neue Touren werden zusätzlich als Geo-Skizze ab Valluhn dargestellt.")
    st.download_button(
        "Feiertags_Wochenplaner.html herunterladen",
        data=html.encode("utf-8"),
        file_name="Feiertags_Wochenplaner_V23.html",
        mime="text/html",
