"""Feiertags-Wochenplaner V27 – Streamlit-Generator für eine eigenständige Dispo-HTML.
...
def build_html(customers, tour_starts, source_name, source_xlsx, day_col_map,
               info_name="", starts_name="", ref_name="") -> str:
...
<title>Feiertags-Wochenplaner V27</title>
<style>
...
<div class="top"><div class="topin">
  <div class="headrow">
    <div><h1>Feiertagsplanung</h1><div class="sub">Woche verschieben · prüfen · exportieren · V27</div></div>
...
<main>
  <section class="plannerbar">
    <div class="plannerflow">
      ...
      <div class="flowhint">Vorschläge in Gruppen: Wochen-Tour → Tour-DNA → Tourfamilie → Zieltag-Tour → Geo → Zeit</div>
    </div>
  </section>
...
const SERVICE_MIN=12,LOAD_MIN=60,AUTO_OVER=2,ROAD_FACTOR=1.28,AVG_KMH=62,MAPS_PER_SECTION=9;
...
function travelMinutes(a,b){const d=dist(a,b);return Number.isFinite(d)?d*ROAD_FACTOR/AVG_KMH*60:NaN}
...
function evalSequence(items,r){
  const timed=hasRouteStart(r);let t=departureMinutes(r),pos=DEPOT,km=0,viol=0;const etas=[];
  items.forEach(a=>{const tr=travelMinutes(pos,a);if(Number.isFinite(tr)){t+=tr;km+=dist(pos,a)}const w=timeWindowFor(a,r.day);if(w.earliest!=null&&t<w.earliest)t=w.earliest;const late=timed&&w.latest!=null&&t>w.latest;if(late)viol++;etas.push({arr:timed?t:NaN,late});t+=SERVICE_MIN;if(hasGeo(a))pos=a});
  km+=d0(pos,DEPOT);return {etas,viol,km};
}
...
function orderByTemplate(items,r){
  ...
  free.sort((x,y)=>d0(y,DEPOT)-d0(x,DEPOT));
  free.forEach(a=>{if(!hasGeo(a)){seq.push(a);return}let best=seq.length,bc=Infinity;for(let i=0;i<=seq.length;i++){const p=i===0?DEPOT:seq[i-1],n=i===seq.length?DEPOT:seq[i];const c=d0(p,a)+d0(a,n)-d0(p,n);if(c<bc){bc=c;best=i}}seq.splice(best,0,a)});
  return seq;
}
...
function bestOrder(r){
  const tpl=orderByTemplate(r.items,r),et=evalSequence(tpl,r);
  if(!et.viol)return {items:tpl,ev:et,mode:'template'};
  const gr=greedySchedule(r.items,r),eg=evalSequence(gr,r);
  if(eg.viol<et.viol)return {items:gr,ev:eg,mode:'greedy'};
  return {items:tpl,ev:et,mode:'template'};
}
function applyEval(r,ev){r.items.forEach((a,i)=>{a._eta=ev.etas[i]?.arr;a._timeViolation=!!ev.etas[i]?.late;a.currentLoadOrder=i+1});r.timeViolations=ev.viol;r.km=ev.km}
...
function evaluateRoute(r){if(r&&r.items.length)applyEval(r,evalSequence(r.items,r))}

/* ---------- Tour-DNA / Tourfamilien ---------- */
function affinity(a,b){if(sapOf(a)===sapOf(b))return 0;const A=SAP_ROUTES.get(a.Quelle+'|'+sapOf(a)),B=SAP_ROUTES.get(b.Quelle+'|'+sapOf(b));if(!A||!B)return 0;let s=0;A.forEach(x=>{if(B.has(x))s++});return s/Math.min(A.size,B.size)}
...
function buildOriginalPlan(){
  assignments=[];routes=[];unplanned=[];clusters=[];cancelledAids=new Set();cancelReasons=new Map();autoCoveredAids=new Set();SAP_ROUTES=new Map();FAM_CACHE.clear();
...
function fitFor(a,r,skipTime=false){
  ...
  let nearest=Infinity,near=null;geoOthers.forEach(x=>{const d=dist(a,x);if(d<nearest){nearest=d;near=x}});
  const centroid=dist(a,centroidOf(geoOthers));
  let key,label;
  if(nearest<=12&&centroid<=35){key='good';label='passt sehr gut'}else if(nearest<=25&&centroid<=55){key='ok';label='passt gut'}else if(nearest<=45&&centroid<=80){key='warn';label='grenzwertig'}else{key='bad';label='eher nicht'}
...
function clusterHtml(c){
  ...
  meta.push(esc(srcLabel(c.source))+' · aus '+esc(origins));if(Number.isFinite(c.km))meta.push('ca. '+Math.round(c.km)+' km Rundlauf (Luftlinie)');
...
function cardHtml(a,r,pos){
  ...
  if(moved){geo=`<div class="geoline"><span class="fit ${fit.key}">${esc(fit.label)}${Number.isFinite(fit.nearest)?' · '+fmt(fit.nearest)+' km':''}</span>${fit.peers>=2?`<span class="flag dna">DNA ${fit.peers}</span>`:''}${fit.dupe?'<span class="flag dupe">Doppellieferung</span>':''}${a.Zeitkritisch?`<span class="flag time" title="${esc(timeInfo(a))}">Zeit</span>`:''}${fit.startMismatch?'<span class="flag area">Startbereich!</span>':''}</div>`}
...
function routeHtml(r){
  ...
  const meta=`${r.items.length}/${maxCustomers} Kunden`+(clock?' · '+clock:'')+(r.repLabel?' · Vorlage '+esc(r.repLabel):'')+(tc?` · ⚠ ${tc} Zeitkonflikt(e)`:'');
...
function renderMetrics(){
  ...
  set('mCap',routes.filter(r=>r.items.length>maxCustomers).length);set('mTime',routes.reduce((n,r)=>n+r.items.filter(a=>a._timeViolation).length,0));
  updateOutageStatus();
}
...
function buildReportHtmlBody(){
  ...
  `<div class="reportsection"><h3>Neue Touren</h3>${reportTable(v.newRoutes.map(r=>[DAY_LABELS[r.day],r.tour,srcLabel(r.source),r.items.length,r.familyLabel||'–',r.repLabel||'–',hasRouteStart(r)?formatMin(routeStartMinutes(r)):'',hasRouteStart(r)?formatMin(departureMinutes(r)):'',[...new Set(r.items.map(a=>DAY_SHORT[a.originalDay]+' '+a.originalTour))].join(', '),r.items.filter(a=>a._timeViolation).length]),['Tag','Tour','Startbereich','Kunden','Tourfamilie','Vorlage','Ladebeginn','Abfahrt','Herkunft','Zeitkonflikte'])}</div>`+
...
function renderBacktest(){
  ...
  document.getElementById('backtestHint').textContent=`Referenz: ${REF_NAME} · Ausfall ${outageDaysLabel()} → ${DAY_LABELS[document.getElementById('toDay').value]} · verglichen wird der aktuelle Plan.`;
...
function runAutoplan(){
  ...
  buildOriginalPlan();clusters=buildClusters(selectedOutageDays(),document.getElementById('toDay').value);applyClusters(()=>true,true);renderAll();return true;
}
...
def main():
    ...
    st.set_page_config(page_title="Feiertags-Wochenplaner V27", page_icon="📅", layout="wide")
    st.title("Feiertags-Wochenplaner V27 – HTML Generator")
    st.caption(
    ...
    try:
        html = build_html(
            ...
            get_day_column_map(source_bytes), info_upload.name, starts_upload.name, ref_name,
        )
    except Exception as exc:
        st.error(f"Fehler: {exc}")
        return
    ...
    m[2].metric("mit Zeit-Hinweis", int(geo["Zeitkritisch"].sum()))
    m[3].metric("ohne Geo-Koordinate", int(geo[["lat", "lon"]].isna().any(axis=1).sum()))
    m[4].metric("Referenz-Treffer", f"{matched}/{len(geo)}" if matched is not None else "–")

    st.download_button(
        "Feiertags_Wochenplaner_V27.html herunterladen",
        data=html.encode("utf-8"), file_name="Feiertags_Wochenplaner_V27.html",
